...
4. **初回コールド起動時の再描画取りこぼし** — `notifyOutputChanged()`をデータセットロードと同期的なタイミングで呼ぶと、プラットフォームの再描画シグナルが取りこぼされ「地図の設定を読み込んでいます...」のまま止まる(ハードリロードでのみ回避可能)不具合。`setTimeout`でマクロタスクまで呼び出しを遅延させて解消
